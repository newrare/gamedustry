...
**A game may also COUNT its own stars**, with `Game.levelTally()` — 0 to 3,
the stars the round holds right now. Where a game has it, it REPLACES the score
bands rather than capping them: the pill lights what the tally says, the bar
shows it out of three, and the end screen gets no objective row, since there is
no number to meet — the game's own rows say what earned each star. It is for a
round that is won on objectives and not on a quantity: `games/stratideck` pays
one star for the flag, one for a battle without a wound and one for a prisoner,
