...
/*
  extract — split the motor out of the games, once.
    ...
    node tools/build/extract.mjs            # write packages/ and the per-game sources
    node tools/build/extract.mjs --dry      # report only, touch nothing
  ...
  `tools/build/build.mjs --check` asserts the round trip is byte-exact. Run the
...
async function main() {
  ...
  console.log(`\nNow verify the round trip:  node tools/build/build.mjs --check`);
}
