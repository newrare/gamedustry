...
│   ├── webshell/             ← web only: six views, six cards, one wallet band
│   │   ├── view.js  view.css ← what a view is, what a modal is, the stack
│   │   ├── menu.js  menu.css ← the title screen, the ranking, options, help
│   │   ├── levels.js  .css   ← the map, the 30 levels, the stars
│   │   ├── meta.js  meta.css ← the wallet, the rewards, the ceremonies
│   │   ├── album.js          ← the collection, the machine, the shop
│   │   └── daily.js          ← the daily road: a house of the village
...
│   ├── pawko/index.html           ← Pawko (drop waves of balls, play a card between)
│   ├── stratideck/index.html      ← Stratideck (drag a card onto a hidden enemy, rank wins)
...
│   └── test/
│       └── views.mjs        ← the web shell's views and cards, in a real build
└── docs/
    ...
    └── AUDIT.md              ← the 2026-09 code audit: findings, fixes, order
```
...
| `pawko` (Pawko)           | A pachinko board of brass pegs under a chute and five cards over it; a round is five waves of balls, one card is played before each — pegs turned to fire, ice, bumpers, magnets or bombs for good, double pegs, double gates or a fatter wave for one wave — and a tap picks where the wave falls; every peg touched pays, a hot, cold or charged ball pays more, and the gate a ball ends in adds or takes a bonus                                                 | Tap to drop    | 5 waves / cards                     |
| `stratideck` (Stratideck) | A camp of face-down enemy cards on a grid that grows with the level, and your own army dealt four at a time under it; drag a card onto an enemy you can reach and the ranks decide — higher wins, equal both fall, the spy kills the marshal, the sapper alone clears a trap, the scout reveals instead of fighting — a winner comes back to the deck, a loser is gone, and the hidden flag ends the battle in a capture                                             | Drag to strike | until the camp or the army is spent |
